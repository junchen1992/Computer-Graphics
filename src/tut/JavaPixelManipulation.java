...
/**
 * A demo program that manipulates colors of individual pixels in a
 * BufferedImage. The program lets the user draw using some basic shapes. The
 * user can also load an image from a file; the loaded image is scaled to
 * exactly fill the panel. A copy of the panel content is stored in a
 * BufferedImage. The user can apply "filters" such as "Blur" to the image. The
 * filter is computed by hand, using the RGB color data in the BufferedImage.
 * There is also a "Smudge" tool that the user can drag on the image to spread
 * color around like wet paint; it works with the pixel data from the
 * BufferedImage.
 */
public class JavaPixelManipulation extends JPanel {
	...
	/**
	 * The main routine simply opens a window that shows a JavaPixelManipulation
	 * panel. The window is fixed size, and the size is set to allow the panel
	 * to have its preferred size.
	 */
	...
	private BasicStroke stroke; // The current stroke, used for lines and
								// curves.

	private Image saveLoadedImage = null; // Keeps a copy of the last loaded
											// image,
											// for convenience. A "Reload Image"
											// menu
	...
	private String dragShape = null; // When non-null, the user is dragging with
										// the Oval, Rectangle, or Line tool.
										// The
										// current shape is drawn in
										// paintComponent()
										// over the BufferedImage. The shape is
										// only
										// added to the image when the drag
										// action ends.
	private int dragStartX, dragStartY; // Start point of drag for use with
										// dragShape.
	private int dragCurrentX, dragCurrentY; // Current mouse position for use
											// with dragShape.

	private double[][] smudgeRed, smudgeBlue, smudgeGreen; // Data used by
															// "Smudge" tool.

	/**
	 * The constructor sets the preferred size of the panel, creates the
	 * BufferedImage, and installs a mouse listener on the panel to implement
	 * drawing actions.
	 */
	public JavaPixelManipulation() {
		...
		addMouseListener(new MouseHandler()); // nested class MouseHandler is
												// defined below.
	}

	/**
	 * The paintComponent() copies the BufferedImage to the screen. If the user
	 * is dragging with the "Line", "Rectangle", or "Oval" tool, the shape is
	 * drawn over the image from the BufferedImage.
	 */
	...
	/**
	 * For drawing the Line, Rectangle, or OvalShape defined by the two points
	 * (x1,y1) and (x2,y2). If the repaint parameter is true, then the panel's
	 * repaint() method is called for a rectangle that contains the shape. When
	 * this method is used to draw to the BufferedImage, repaint is true, so
	 * that the change to the BufferedImage will also be shown on the screen.
	 * When it is called to draw the dragShape in paintComponent(), repaint is
	 * false.
	 */
	private void putShape(Graphics2D g, String shape, int x1, int y1, int x2, int y2, boolean repaint) {
		...
		if (repaint) {
			repaint(x - 13, y - 13, w + 26, h + 26); // large enough to contain
														// widest stroke
		}
	}
	...
	private void applyTool(String tool, int x, int y) {
		if (tool.equals("Erase")) { // Clear a 10-by-10 square, centered at
									// (x,y).
			OSG.setColor(Color.WHITE);
			OSG.fillRect(x - 5, y - 5, 10, 10); // Erase the sqaure in the
												// BufferedImage.
			repaint(x - 5, y - 5, 10, 10); // Make change visible on the screen.
		} else { // For the "Smudge" tool, mix some of the "paint" on the tool
					// with the image,
	...
	/**
	 * Copy pixel colors from a 7-by-7 square centered at (x,y) into the smudge
	 * data arrays. The color components are separated into their red, green,
	 * and blue components, which are stored in separate arrays. The values are
	 * stored as type double, not int, since they will be used in averaging
	 * calculations that require real arithmetic. This method is called at the
	 * point where the user starts a drag operation with the "Smudge" tool.
	 */
	...
	/**
	 * Swap some of the color stored in the smudge data arrays with color in a
	 * 7-by-7 square centered at (x,y) in the BufferedImage. That is, the color
	 * values in the arrays are replaced by a weighted average of the color
	 * values in the arrays and the color values in the image. At the same time,
	 * the color values in the image are replaced by a weighted average of the
	 * color values in the image and the color values in the arrays. This method
	 * is called at each point along the path that the mouse visits as the user
	...
	private void swapSmudgeData(int x, int y) {
		...
		for (int i = 0; i < 7; i++) { // row number in the smudge data arrays
			int c = x + i - 3; // column number (x-coord) of a pixel in the
								// image.
			for (int j = 0; j < 7; j++) { // column number in the smudge data
											// arrays
				int r = y + j - 3; // row number (y-coord) of a pixel in the
									// image
				if (!(r < 0 || r >= h || c < 0 || c >= w || smudgeRed[i][j] == -1)) {
					int curCol = OSC.getRGB(c, r); // Current color of the pixel
													// in the image.
					int curRed = (curCol >> 16) & 0xFF; // RGB components from
														// image
					int curGreen = (curCol >> 8) & 0xFF;
					int curBlue = curCol & 0xFF;
					int newRed = (int) (curRed * 0.7 + smudgeRed[i][j] * 0.3); // New
																				// RGB's
																				// for
																				// image.
					...
					OSC.setRGB(c, r, newCol); // Replace the color of the pixel
												// in the image.
					smudgeRed[i][j] = curRed * 0.3 + smudgeRed[i][j] * 0.7; // New
																			// RGBs
																			// for
																			// smudge
																			// arrays
	...
	/**
	 * For the "Smudge" and "Erase" tools, apply the tool to every point along
	 * the line from (x1,y1) to (x2,y2). This is called each time the mouse
	 * moves as the user drags the tool.
	 */
	...
	private class MouseHandler extends MouseAdapter {
		boolean dragging = false; // Set to true if a dragging operation is in
									// progress.
		int startX, startY; // The point at which the drag action started.
		int prevX, prevY; // The location of the mouse the previous time
							// mousePressed
							// or mouseDragged was called.

		public void mousePressed(MouseEvent evt) {
			if (dragging)
				return; // There is already a mouse drag in progress; don't try
						// to start a new one.
			dragging = true;
			...
			if (tool.equals("Line") || tool.equals("Oval") || tool.equals("Rectangle")) {
				...
				applyTool("Erase", startX, startY); // Erase a square around the
													// starting point.
			} else if (tool.equals("Smudge")) {
				grabSmudgeData(startX, startY); // Get data from the image that
												// is needed for the Smudge
												// tool.
			}
			addMouseMotionListener(this); // Monitor mouse moves during the drag
											// operation.
		}
		...
		public void mouseDragged(MouseEvent evt) {
			...
			if (tool.equals("Line") || tool.equals("Oval") || tool.equals("Rectangle")) {
				dragCurrentX = x;
				dragCurrentY = y;
				repaint(); // paintComponent() will draw the current shape on
							// top of the image content.
			} else if (tool.equals("Sketch"))
				putShape(OSG, "Line", prevX, prevY, x, y, true); // Draw line
																	// segment
																	// directly
																	// in
																	// BufferedImage.
			else
				applyToolAlongLine(tool, prevX, prevY, x, y); // For Smudge and
																// Erase tools.
			prevX = x;
			prevY = y;
		}
		...
		public void mouseReleased(MouseEvent evt) {
			if (!dragging)
				return;
			removeMouseMotionListener(this); // Stop monitoring mouse motions,
												// since drag is ending.
			dragging = false;
			dragShape = null; // paintComponent will no longer draw the extra
								// shape
			if (tool.equals("Line") || tool.equals("Oval") || tool.equals("Rectangle")) {
				putShape(OSG, tool, startX, startY, prevX, prevY, true); // add
																			// shape
																			// to
																			// BufferedImage
				repaint(); // should be unnecessary; just to be sure that the
							// panel shows the right thing.
			}
		}
	}

	/**
	 * Apply one of the image filters from the "Filter" menu to the
	 * BufferedImage, and copy the result to the screen. A filters is
	 * implemented as a "convolution" with 3-by-3 arrays. That is, the RGB
	 * components of each pixel in the image is replaced with a weighted average
	 * of the RGB components of the pixels in a 3-by-3 square. The weighting
	 * factors are given by the convolution array. For example, for the "Blur"
	 * filter, all the weight factors in the array are equal, and the filter is
	 * just a simple averaging operation. (To make things easy on myself, I
	 * don't change the colors of the pixels along the border of the image; this
	 * lets me assume that when I work on a pixel, the 3-by-3 square centerd at
	 * that pixel is entirely within the image.) The filter must be one of the
	...
	private void applyFilter(String filter) {
		...
		int[] rgbArray = new int[w * h]; // An array to hold the RGB colors of
											// the entire image.
		OSC.getRGB(0, 0, w, h, rgbArray, 0, w); // Grab the RGB color data from
												// the image.
		double v;
		switch (filter) {
		case "Blur":
	...
	private void loadImageFile() {
		...
		try {
			...
			g.dispose();
			repaint();
			saveLoadedImage = image; // Keep a copy of the image so it can be
										// reused with "Reload Image" command.
			reloadImageMenuItem.setEnabled(true); // Enable the "Reload Image
													// command.
		} catch (Exception e) {
	...
	/**
	 * Create the menus for the program, and provide listeners to implement the
	 * menu commands.
	 */
	private JMenuBar getMenuBar() {
		...
		reloadImageMenuItem.setEnabled(false); // Command will be enabled when
												// an image is loaded.
		file.addSeparator();
		...
		ActionListener clistener = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				case "Gray":
					...
					return;
				case "Custom...":
					Color c = JColorChooser.showDialog(JavaPixelManipulation.this, "Select Drawing Color", color);
					if (c != null) {
						color = c;
					}
				}
